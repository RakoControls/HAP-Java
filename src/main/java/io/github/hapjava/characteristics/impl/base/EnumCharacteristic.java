...
/**
 * Characteristic that exposes an Enum value. Enums are represented as an
 * Integer value in the HomeKit protocol, and classes extending this one must
 * handle the static mapping to an Integer value.
 *
 * @author Andy Lintner
 */
public abstract class EnumCharacteristic<T extends CharacteristicEnum>
     ...
     * Default constructor
     *
     * @param type a string containing a UUID that indicates the type of
     * characteristic. Apple defines a set of these, however implementors can
     * create their own as well.
     * @param format
     * @param description a description of the characteristic to be passed to
    ...
    public EnumCharacteristic(
            ...
            Optional<Runnable> unsubscriber) {
        this(
        type, "int", description, validValues, getter, setter, subscriber, unsubscriber);
    }
    ...
    /**
     * @return the current value of this characteristic, or null if it has no
     * value or can't be fetched
     */
